...
const ts = Date.now();
const userA = {
  name: `LiveAuditorA${ts}`,
  email: `liveauditora${ts}@example.com`,
  password: 'TestPass123!',
};
const userB = {
  name: `LiveAuditorB${ts}`,
  email: `liveauditorb${ts}@example.com`,
  password: 'TestPass123!',
};
...
async function run() {
  console.log(`Launching browser contexts against: ${BASE_URL}...`);
  ...
  // Attach console monitors for debugging
  pageA.on('console', msg => {
    if (msg.type() === 'error') console.log(`[BROWSER][UserA][Error] ${msg.text()}`);
  });
  pageB.on('console', msg => {
    if (msg.type() === 'error') console.log(`[BROWSER][UserB][Error] ${msg.text()}`);
  });

  try {
    console.log('Registering User A on deployed production site...');
    await registerUser(pageA, userA);
    console.log('Registering User B on deployed production site...');
    await registerUser(pageB, userB);

    console.log('Establishing friendship on production...');
    ...
    console.log('Recording active call on production... waiting 30 seconds...');
    ...
    const downloadPath = join(DOWNLOAD_DIR, 'deployed-playback-test.webm');
    await download.saveAs(downloadPath);
    console.log(`Recording downloaded successfully from production to: ${downloadPath}`);
    ...
    console.log('\n================ DEPLOYED RECORDING ANALYSIS REPORT ================');
    ...
    console.log(`[✔] Live file plays back successfully: ${isPlaybackSmooth}`);
    console.log(`[✔] Live video track is present and correct: ${hasVideoTrack}`);
    console.log(`[✔] Live audio track contains active sound: ${hasAudioTrack}`);
    console.log('===========================================================\n');

    if (isPlaybackSmooth && hasVideoTrack && hasAudioTrack) {
      console.log('VERDICT: SUCCESS - Deployed application recording is 100% verified and correct!');
    } else {
      console.error('VERDICT: FAILURE - Corrupt or empty tracks found in live recording!');
      process.exit(1);
    }
